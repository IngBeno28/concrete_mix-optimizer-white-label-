...
def create_pdf_report(dataframe, pie_chart_buf):
    """Create a PDF report with analysis results - Fixed version."""
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", size=12)

    # Header with timestamp and project
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    pdf.cell(200, 10, txt=f"Project: {project_name} | Date: {now}", ln=True, align='C')
    pdf.ln(5)
    pdf.cell(200, 10, txt=APP_TITLE, ln=True, align='C')
    pdf.ln(10)

    # Add mix design results
    ...
    if pie_chart_buf:
        try:
            # Save chart image to temporary file and add to PDF
            ...
            # If chart embedding fails, add a note
            pdf.ln(5)
            pdf.set_font("Arial", "I", 10)
            pdf.cell(200, 8, txt="Note: Visual chart is available in the web interface", ln=True)

    # Footer with watermark and note
    pdf.set_y(-30)
    pdf.set_font("Arial", size=10)
    pdf.cell(200, 10, txt=f"Generated by {CLIENT_NAME} | {FOOTER_NOTE}", ln=True, align='C')

    # Fixed PDF output method - use dest='S' for string output
